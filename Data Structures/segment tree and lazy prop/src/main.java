...
public class main {
	...
	static int query(int l, int r, int num)
	{
		...
		{
			//propogate lazy
			set[num].val += (set[num].right - set[num].left +1) *lazy[num];
			if(set[num].left != set[num].right)
			{
				lazy[2*num] += lazy[num];
	...
	static void updateRange(int l, int r, int val, int num)
	{
		if(lazy[num]!=0)
		{
			set[num].val += (set[num].right - set[num].left +1) *lazy[num];
		...
		if(set[num].left==l && set[num].right ==r)
		{
			set[num].val += (set[num].right - set[num].left +1) *val;
			if(set[num].left != set[num].right)
			{
				lazy[2*num] += val;
				lazy[2*num+1] += val;
		...
		if(r<=mid)
		{
			updateRange(l,r,val,2*num);
		}
		else if(l>mid)
		{
			updateRange(l,r,val,2*num+1);
		}
		else
		{
			updateRange(l, mid,val,2*num);
			updateRange(mid+1,r,val,2*num+1);
		}
		set[num].val = set[2*num].val + set[2*num+1].val;
	}
